...
- ``type``: Use ``MAG`` to have ``pfweight`` read magnetic location files.

- ``mesh.msh``: Name of 3D :ref:`mesh file <meshFile>`.

- ``obs.grv`` The :ref:`data file <magFile>` that contains the observation locations and the observed magnetic anomaly with estimated standard deviation.
